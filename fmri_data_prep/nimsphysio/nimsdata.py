...
import datetime
import traceback
from bson import loads as bloads  # Modified - HJ


log = logging.getLogger(__name__)
warnings.simplefilter('ignore', FutureWarning)

# note: readers/writers.json will never contain datetime, or objects that require bson
...
def parse(path, filetype=None, load_data=False, ignore_json=False, debug=False, **kwargs):
    ...
    if os.path.isfile(path) and tarfile.is_tarfile(path):
        ...
        with tarfile.open(path) as archive:
            for ti in archive:
                try:
                    json_data = bloads(archive.extractfile(ti).read())
...
class NIMSReader(object):
    ...
    def _schema_init(self, schema):
        for k, v in schema.items():  # modified - HJ
